...
function publicDevice(device) {
  ...
  return {
    ...
    themeId: normalizeThemeId(device.themeId),
    unitName: normalizeText(device.unitName || state.settings.unitName || ""),
    unitLogoUrl: sanitizeMediaUrl(device.unitLogoUrl || state.settings.unitLogoUrl || ""),
...
function publicSettings() {
  ...
  return {
    ...state.settings,
    unitName: normalizeText(state.settings.unitName || ""),
    unitLogoUrl: sanitizeMediaUrl(state.settings.unitLogoUrl || ""),
    telegram: {
...
function normalizeRestoredDevice(device) {
  return {
    ...createDevice({
      ...
      unitLogoUrl: device.unitLogoUrl,
...
function sanitizeUploadFilename(filename, contentType = "") {
  ...
  const allowedExtensions = [...DIRECT_VIDEO_EXTENSIONS, ".png", ".jpg", ".jpeg", ".webp"];
  const extension = allowedExtensions.includes(extFromName) ? extFromName : (extFromType || ".mp4");
  const fallbackBase = [".png", ".jpg", ".jpeg", ".webp"].includes(extension) ? "logo" : "video";
  const base = (path.basename(original, extFromName || undefined) || fallbackBase)
...
function createDevice(body, activationToken = token()) {
  return {
    ...
    unitName: normalizeText(body.unitName || ""),
    unitLogoUrl: sanitizeMediaUrl(body.unitLogoUrl || ""),
...
async function handleApi(req, res, url) {
  ...
  if (deviceMatch && req.method === "PATCH") {
    ...
    if (body.themeId !== undefined) device.themeId = normalizeThemeId(body.themeId);
    if (body.unitName !== undefined) device.unitName = normalizeText(body.unitName || "");
    if (body.unitLogoUrl !== undefined) device.unitLogoUrl = sanitizeMediaUrl(body.unitLogoUrl);
  ...
  if (req.method === "PATCH" && url.pathname === "/api/settings") {
    ...
    state.settings = {
      ...state.settings,
      ...body,
      unitName: normalizeText(body.unitName !== undefined ? body.unitName : state.settings.unitName || ""),
      unitLogoUrl: sanitizeMediaUrl(body.unitLogoUrl !== undefined ? body.unitLogoUrl : state.settings.unitLogoUrl || ""),
      telegram: {
        ...(state.settings.telegram || {}),
        ...(body.telegram || {})
      }
